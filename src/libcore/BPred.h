...
#include "GStats.h"
#include "libll/Instruction.h"        // InstID typedef uint32_t InstID;
#include "CacheCore.h"
#include "EnergyMgr.h"
#include "SCTable.h"
...
// Defining Max size of array for easier maniplation
#define MAX_INSTURCTION 5000
...
// Parin Code Starts
// Defining My Varaibles
    uint count ;
    uint counter[MAX_INSTURCTION] ; //total counter 
    uint missed[MAX_INSTURCTION] ; // total mis prediction counter
    uint hit[MAX_INSTURCTION] ; // total correct prediction counter
    InstID total_InstID[MAX_INSTURCTION]; // instruction ID info
    int Binstruction[4]; // Defining counter for 1-9 ,10-99,100-999,...
    int Bhit_counter[4]; // Hit Accuracy counter for ^^
