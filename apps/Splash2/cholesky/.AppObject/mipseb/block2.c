...
int *T, *firstchild, *child, *PERM, *INVP, *domain, *partition;
{
  ...
  LB.entry = (Entry *) our_malloc(LB.entries_allocated*sizeof(Entry),__FILE__,__LINE__);;
  MigrateMem(LB.entry, LB.entries_allocated*sizeof(Entry), DISTRIBUTED);
  LB.row = (int *) our_malloc(LB.entries_allocated*sizeof(int),__FILE__,__LINE__);;
...
/* Generated from ../Source/block2.C */
