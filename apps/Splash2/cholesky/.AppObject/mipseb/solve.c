...
char *argv[];
{
  ...
  gp = (struct gpid *) our_malloc(sizeof(struct gpid),__FILE__,__LINE__);;
  gp->pid = 0;
  Global = (struct GlobalMemory *)
    our_malloc(sizeof(struct GlobalMemory),__FILE__,__LINE__);;
  {
...
pthread_create(&(__tid__[__threads__++]), NULL, (void*(*)(void *))(Go), NULL);
...
/* POSSIBLE ENHANCEMENT:  Here is where one might pin processes to
  ...
  lc =(struct LocalCopies *) our_malloc(sizeof(struct LocalCopies)+2*PAGE_SIZE,__FILE__,__LINE__);
  lc->freeUpdate = NULL;
  lc->freeTask = NULL;
  lc->runtime = 0;
