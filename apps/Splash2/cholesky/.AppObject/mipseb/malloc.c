...
MallocInit(P)
{
  ...
  mem_pool = (struct MemPool *)
    our_malloc((mallocP+1)*sizeof(struct MemPool),__FILE__,__LINE__);;
  mem_pool++;
...
InitOneFreeList(p)
{
  ...
  if (p > 0) {
    mem_pool[p].freeBlock = (unsigned **)
      our_malloc((MAXFAST+1)*sizeof(unsigned *),__FILE__,__LINE__);;
...
int size, home;
{
  ...
  if (result) {
    ...
    freespace = (unsigned *) our_malloc(block_size+2*sizeof(unsigned),__FILE__,__LINE__);;
...
/* Generated from ../Source/malloc.C */
