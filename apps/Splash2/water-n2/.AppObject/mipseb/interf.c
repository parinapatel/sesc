  ...
  unsigned ProcID;
{
    ...
    *VIR = *VIR + LVIR;
    ...
    {
 pthread_mutex_lock(&((gl->InterfBar).bar_mutex));
 (gl->InterfBar).bar_teller++;
 if ((gl->InterfBar).bar_teller == (NumProcs)) {
     (gl->InterfBar).bar_teller = 0;
     pthread_cond_broadcast(&((gl->InterfBar).bar_cond));
 } else
     pthread_cond_wait(&((gl->InterfBar).bar_cond), &((gl->InterfBar).bar_mutex));
 pthread_mutex_unlock(&((gl->InterfBar).bar_mutex));
};
...
/* Generated from ../Source/interf.C */
