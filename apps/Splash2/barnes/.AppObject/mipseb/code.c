...
pthread_mutex_unlock(&__intern__);
   ...
   {int aantal; for(aantal=NPROC-1;aantal>0;aantal--) pthread_join(__tid__[--__threads__], NULL);};
...
tab_init()
{
   ...
   for (i = 0; i < NPROC; ++i) {
      Local[i].ctab = (cellptr) our_malloc((maxcell / NPROC) * sizeof(cell),__FILE__,__LINE__);;
      Local[i].ltab = (leafptr) our_malloc((maxleaf / NPROC) * sizeof(leaf),__FILE__,__LINE__);;
   }

   /*allocate space for personal lists of body pointers */
   maxmybody = (nbody+maxleaf*MAX_BODIES_PER_LEAF)/NPROC; 
   Local[0].mybodytab = (bodyptr*) our_malloc(NPROC*maxmybody*sizeof(bodyptr),__FILE__,__LINE__);;
   ...
   Local[0].mycelltab = (cellptr*) our_malloc(NPROC*maxmycell*sizeof(cellptr),__FILE__,__LINE__);;
   Local[0].myleaftab = (leafptr*) our_malloc(NPROC*maxmyleaf*sizeof(leafptr),__FILE__,__LINE__);;

   CellLock = (struct CellLockType *) our_malloc(sizeof(struct CellLockType),__FILE__,__LINE__);;
