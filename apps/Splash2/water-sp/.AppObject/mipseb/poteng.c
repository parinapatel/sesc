  ...
  
{
    ...
    {
 pthread_mutex_lock(&((gl->PotengBar).bar_mutex));
 (gl->PotengBar).bar_teller++;
 if ((gl->PotengBar).bar_teller == (NumProcs)) {
     (gl->PotengBar).bar_teller = 0;
     pthread_cond_broadcast(&((gl->PotengBar).bar_cond));
 } else
     pthread_cond_wait(&((gl->PotengBar).bar_cond), &((gl->PotengBar).bar_mutex));
 pthread_mutex_unlock(&((gl->PotengBar).bar_mutex));
};
...
/* Generated from ../Source/poteng.C */
