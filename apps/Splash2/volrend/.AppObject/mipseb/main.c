...
{	
  ...
  if (num_nodes > 1)
    {int aantal; for(aantal=num_nodes-1;aantal>0;aantal--) pthread_join(__tid__[--__threads__], NULL);};
  {exit(0);};
}	
...
Frame()
{
  ...
  {
pthread_mutex_init(&((Global->SlaveBarrier).bar_mutex), NULL);
pthread_cond_init(&((Global->SlaveBarrier).bar_cond), NULL);
(Global->SlaveBarrier).bar_teller=0;
};
  {
pthread_mutex_init(&((Global->TimeBarrier).bar_mutex), NULL);
pthread_cond_init(&((Global->TimeBarrier).bar_cond), NULL);
(Global->TimeBarrier).bar_teller=0;
};
  {pthread_mutex_init(&(Global->IndexLock),NULL);};
  {pthread_mutex_init(&(Global->CountLock),NULL);};
  { int i; for(i = 0; i < (MAX_NUMPROC+1); i++) pthread_mutex_init(&((Global->QLock)[i]), NULL); };
...
pthread_create(&(__tid__[__threads__++]), NULL, (void*(*)(void *))(Render_Loop), NULL);
     ...
     long length;
{
  ...
  *address = (PIXEL *)our_malloc(length*sizeof(PIXEL),__FILE__,__LINE__);;
     ...
     long length;
{
  ...
  *address = (MPIXEL *)our_malloc(length*sizeof(MPIXEL),__FILE__,__LINE__);;
...
/*  POSSIBLE ENHANCEMENT:  If you want to replicate the shade table,
  ...
  *address1 = (PIXEL *)our_malloc(length,__FILE__,__LINE__);;
