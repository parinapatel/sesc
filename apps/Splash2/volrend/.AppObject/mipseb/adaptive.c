...
Ray_Trace(int my_node)
{
  ...
  if (adaptive) {
    ...
    {
 pthread_mutex_lock(&((Global->TimeBarrier).bar_mutex));
 (Global->TimeBarrier).bar_teller++;
 if ((Global->TimeBarrier).bar_teller == (num_nodes)) {
     (Global->TimeBarrier).bar_teller = 0;
     pthread_cond_broadcast(&((Global->TimeBarrier).bar_cond));
 } else
     pthread_cond_wait(&((Global->TimeBarrier).bar_cond), &((Global->TimeBarrier).bar_mutex));
 pthread_mutex_unlock(&((Global->TimeBarrier).bar_mutex));
};
...
/* Generated from ../Source/adaptive.C */
